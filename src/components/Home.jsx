...
import { rootRoute } from "../main"; // Adjust the import path according to your project structure
...
const Home = () => {
  ...
  // Extract items and hasMore from data (destructuring with default values)
                ...
                <div className="lg:grid lg:grid-cols-5 flex flex-col h-full w-full gap-4">
                  {items.length > 0 ? (
                    items.map((item) => (
                      <div
                        key={item.id}
                        className="h-full w-full border flex flex-col justify-between p-4 bg-slate-200 rounded-md cursor-pointer shadow-xl transform transition-transform duration-300 ease-in-out hover:scale-95"
                      >
